...
function addMaintainerCommand(names, cb, flags) {
    addCommand(3, names, cb, (flags || 0) | commandFlags.MAINTAINERS);
}

function addCheatCode(names, cb, flags) {
    addCommand(-1, names, cb, (flags || 0) | commandFlags.MAINTAINERS | commandFlags.HIDDEN);
}

function addChannelModCommand(names, cb, flags) {
    addCommand(1, names, cb, (flags || 0) | commandFlags.CHANNELMODS);
}

function addChannelAdminCommand(names, cb, flags) {
    addCommand(2, names, cb, (flags || 0) | commandFlags.CHANNELADMINS);
}

function addChannelOwnerCommand(names, cb, flags) {
    addCommand(3, names, cb, (flags || 0) | commandFlags.CHANNELOWNERS);
}
...
addListCommand(1, "megausercommands", "Megauser", null, commandFlags.MEGAUSERS);
...
addCommand(0, "burn", function (src, command, commandData, tar, chan) {
    ...
    if (!tar) {
        return bot.sendMessage(src, "Target doesn't exist!", chan);
    }

    broadcast("<img src=Themes/Classic/status/battle_status4.png><b style='color:red'>" + Utils.escapeHtml(sys.name(tar)) + " was burned by " + Utils.escapeHtml(sys.name(src)) + "</b><img src=Themes/Classic/status/battle_status4.png>", chan);
});
...
addCommand(0, "freeze", function (src, command, commandData, tar, chan) {
    ...
    broadcast("<img src=Themes/Classic/status/battle_status3.png><b style='color:blue'>" + Utils.escapeHtml(sys.name(tar)) + " was frozen by " + Utils.escapeHtml(sys.name(src)) + "</b><img src=Themes/Classic/status/battle_status3.png>", chan);
});
...
addCommand(0, "paralyze", function (src, command, commandData, tar, chan) {
    ...
    broadcast("<img src=Themes/Classic/status/battle_status1.png><b style='color:#C9C909'>" + Utils.escapeHtml(sys.name(tar)) + " was paralyzed by " + Utils.escapeHtml(sys.name(src)) + "</b><img src=Themes/Classic/status/battle_status1.png>", chan);
});
...
addCommand(0, ["sendto", "ping"], function (src, command, commandData, tar, chan) {
    ...
    if (!tar) {
        return bot.sendMessage(src, "Must send the message to a real person!", chan);
    }
...
addCommand(0, "attack", function (src, command, commandData, tar, chan) {
    ...
    broadcast("<font color=green><timestamp/><b><i>+AttackBot:</i></b></font> <b style='color:" + Utils.nameColor(src) + ">" + Utils.escapeHtml(sys.name(src)) + " </b> has used <b style='color:" + Color.randomDark() + "'>" + sys.move(move)) + "</b> on <b style='color:" + Utils.nameColor(tar) + ">" + Utils.escapeHtml(sys.name(tar)) + "!</b>", chan);
});
...
addCommand(0, "spin", function (src, command, commandData, tar, chan) {
    ...
    if (spinTypes.indexOf('emotes') !== -1) {
        possibilities.push("<b><font color=" + Utils.nameColor(src) + ">" + sys.name(src) + "</b></font> has spun a <font color=gray><b>" + sys.rand(1, 9002) + "</b></font> and won <img src='" + Emotes.code(randomEmote) + "'>!");
    }
