Utils = require './utils'
Config = require './config'
Server = require './server'
RTD = require './rtd'

emotes = {}
names = []
list = []

emojis = {}
emojiRegex = /:([a-z0-9\+\-_]+):/g

poEmotes = /((trainer|icon|item|pokemon):([(\d|\-)&=(gen|shiny|gender|back|cropped|num|substitute|true|false)]+))/g

emoteRegexCache = {}
nonAlpha = /\W/
marxState = 0
marxmotes = "marx1 lenin1 stalin1".split("")

loaded = no

code = (name) -> emotes[name]
random = -> names[sys.rand(0, names.length)]
add = (alts, code) ->
    unless Array.isArray(alts)
        alts = [alts]

    for alt in alts
        names.push alt
        emotes[alt] = code

        regex = Utils.escapeRegex(alt)
        unless nonAlpha.test(alt)
            regex = "\\b" + regex + "\\b"
        emoteRegexCache[alt] = new RegExp(regex, "g")

    list.push alts.join(" | ")

format = (message, limit, src) ->
    unless Config.emotesEnabled
        return message

    load()

    #perm = Utils.mod.hasBasicPermissions(src);
    #timeout = perm ? 4 : 7;

    usedEmotes = []
    lastEmote = []
    emojiCount = 0
    uobj = SESSION.users(src)
    timeout = 3
    time = sys.time()
    size = ""

    assignEmote = (emote) ->
        return ($1) ->
            if usedEmotes.length > 4 or (limit and lastEmote.indexOf(emote) isnt -1)
                return Utils.escapeHtml($1)

            usedEmotes.push emote
            if limit and uobj?.lastEmote
                uobj.lastEmote.push emote

            if Server.marxmode and emote in marxmotes
                marxState += 1
                if marxState is 1
                    emote = "marx1"
                else if marxState is 2
                    emote = "stalin1"
                    code = Emotes.code("stalin1")
                else
                    emote = "lenin1"
                    marxState = 0
                emotecode = code(emote)
            else if Server.georgemode
                emote = "george1"
                emotecode = code("george1")
            else if src
                if RTD.hasEffect(src, "blank_emotes")
                    emotecode = emote = "invalid"
                    size = " width='50' height='50'"
                else if RTD.hasEffect(src, "nobody_cares") # Sorry for the inline html!
                    emote = "nc"
                    emotecode = "data:image/gif;base64,"
                else if RTD.hasEffect(src, "im_blue")
                    emote = "blu2"
                    emotecode = "data:image/png;base64,"
                else if RTD.hasEffect(src, "random_emotes")
                    emote = random()
                    emotecode = code(emote)

            unless emotecode
                emotecode = code(emote)

            return "<a href='po:appendmsg/ #{emote}'><img title='#{emote}' src='#{emotecode}'#{size}></a>"

    if limit and src and uobj
        if uobj.lastEmoteTime and uobj.lastEmoteTime + timeout > time
            lastEmote = uobj.lastEmote or []
        else
            uobj.lastEmote = []
    if src
        if RTD.hasEffect(src, "bigger_emotes")
            size = " width='100' height='100'"
        else if RTD.hasEffect(src, "smaller_emotes")
            size = " width='25' height='25'"

    # First, pokemons, icons, items, and avatars.
    # pokemon:subtitute|pokemon:30&cropped=true
    message = message.replace(poEmotes, "<a href='po:appendmsg/ $1'><img src='$1'  title='$1'></a>")
    for emote of Emotes.list
        if usedEmotes.length > 4
            break

        # Major speed up.
        if message.indexOf(emote) isnt -1
            message = message.replace(emoteRegex[emote], assignEmote(emote))

    # Emoji effects
    if src
        if RTD.hasEffect(src, "bigger_emotes")
            size = " width='44' height='44'"
        else if RTD.hasEffect(src, "smaller_emotes")
            size = " width='11' height='11'"

    message = message.replace emojiRegex, (name) ->
        emoji = name.substr(1, name.length - 2)

        if (usedEmotes.length + emojiCount) > 5
            return name

        if emojis.hasOwnProperty(emoji)
            emojicode = emojis[emoji]

            if src and RTD.hasEffect(src, "blank_emotes")
                emojicode = "invalid"
                size = " width='22' height='22'"

            emojiCount += 1
            return "<img src='#{emojicode}'#{size}>"

        return name

    if limit and uobj?.lastEmote?.toString() isnt lastEmote.toString()
        uobj.lastEmoteTime = time
    return message

# Accepts either a name (the player must be online) or id
hasPermission = (name) ->
    id = sys.id(name) or name
    user = SESSION.users(id)
    if id and user?.originalName
        name = user.originalName
    ip = sys.dbIp(name)

    # TODO: hasEmotePerms, isMaintainer
    if sys.maxAuth(ip) > 0 or Emoteperms.hasOwnProperty(name.toLowerCase()) or name in Config.maintainers
        return no

    aliases = sys.aliases(ip)
    len = aliases?.length
    if len < 1
        return no

    for alias in aliases
        if Emoteperms.hasOwnProperty(alias.toLowerCase())
            return yes
    return no

# Accepts either a name (the player must be online) or id
enabledFor = (src) ->
    id = sys.id(src) or src
    name = SESSION.users(id).originalName.toLowerCase()
    # TODO: hasEmotesEnabled
    return (Utils.mod.hasBasicPermissions(id) or hasPermission(id)) and Emotetoggles.hasOwnProperty(name)

load = ->
    if loaded
        return

    emoji = JSON.parse(sys.synchronousWebCall(Config.emojiurl))
    emoteSource = JSON.parse(sys.synchronousWebCall(Config.emotesurl))
    delete emoteSource["@NOTICE"]

    emotes = {}
    names = []
    list = []
    for emote, src of emoteSource
        add emote.split(","), src

    # Misc emotes
    add ":(", "item:177"
    add ":charimang:", "pokemon:6&gen=2"
    add ":mukmang:", "pokemon:89&gen=1"
    add ":feralimang:", "pokemon:160&gen=2"
    add "oprah1", "pokemon:124&gen=1"
    add "oprah2", "pokemon:124&gen=2"
    list.sort()

    loaded = yes

reload = ->
    loaded = no
    load()

interpolate = (src, msg, vars, checkEnabled=no, rateLimit=yes) ->
    for name, val of vars
        msg = msg.replace(new RegExp(Utils.escapeRegex(name), "gi"), val)

    if !checkEnabled or (checkEnabled and enabledFor(src))
        msg = format(msg, rateLimit, src)
    return msg

module.exports = {
    # API
    code, random, add, format, hasPermission, enabledFor, load, interpolate,
    emotes, names, list,
    emoji: emojis,
    # Enums
    ratelimit: yes
    always: no
}
